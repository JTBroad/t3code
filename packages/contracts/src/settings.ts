...
export const CopilotSettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: makeBinaryPathSetting("copilot").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description:
          "Path to the GitHub Copilot CLI binary. Leave empty to use the SDK-bundled CLI.",
    ...
    homePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        description:
          "Custom Copilot data directory (session state, config). Keeps accounts separate.",
        providerSettingsForm: {
          placeholder: "~/.copilot",
          clearWhenEmpty: "omit",
        },
